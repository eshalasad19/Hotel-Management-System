            ...
            <div class="page-content">
                <div class="container-fluid">
                    <div class="row">
                        <div class="col-12">
                            <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                                <h4 class="mb-sm-0">Housekeeping Management</h4>
                                <button class="btn btn-success" data-bs-toggle="modal" data-bs-target="#addTaskModal">
                                    <i class="ri-add-line me-1"></i> Assign Task
                                </button>
                            </div>
                        </div>
                    </div>
                                ...
                                <div class="card-body">
                                    <div class="d-flex align-items-center">
                                        ...
                                        <div class="flex-grow-1 ms-3">
                                            <p class="text-uppercase fw-medium text-muted mb-1 fs-12">Total Tasks</p>
                                            <h4 class="mb-0 fw-semibold" id="stat-total">0</h4>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="card card-animate">
                                <div class="card-body">
                                    <div class="d-flex align-items-center">
                                        ...
                                        <div class="flex-grow-1 ms-3">
                                            <p class="text-uppercase fw-medium text-muted mb-1 fs-12">In Progress</p>
                                            <h4 class="mb-0 fw-semibold" id="stat-inprogress">0</h4>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="card card-animate">
                                <div class="card-body">
                                    <div class="d-flex align-items-center">
                                        ...
                                        <div class="flex-grow-1 ms-3">
                                            <p class="text-uppercase fw-medium text-muted mb-1 fs-12">Completed</p>
                                            <h4 class="mb-0 fw-semibold" id="stat-completed">0</h4>
                                        </div>
                                    </div>
                                </div>
                            </div>
                    ...
                    <div class="card">
                        <div class="card-header d-flex align-items-center">
                            <h5 class="card-title mb-0 flex-grow-1">Cleaning Tasks</h5>
                ...
                <div class="modal fade" id="addTaskModal" tabindex="-1">
                    <div class="modal-dialog">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h5 class="modal-title">Assign Cleaning Task</h5>
                                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                            </div>
                            <div class="modal-body">
                                <div id="add-task-error" class="alert alert-danger d-none"></div>
                                <div class="row g-3">
                                    <div class="col-12">
                                    ...
                                    <div class="col-12">
                                        <label class="form-label">Assign Staff <span class="text-danger">*</span></label>
                                        <select class="form-select" id="add-staffId">
                            ...
                            <div class="modal-footer">
                                ...
                                <button type="button" class="btn btn-success" id="addTaskBtn">Assign Task</button>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="modal fade" id="updateStatusModal" tabindex="-1">
                    <div class="modal-dialog modal-sm">
                        <div class="modal-content">
                            ...
                            <div class="modal-body">
                                <input type="hidden" id="update-taskId">
                                <div class="mb-3">
                                    <label class="form-label">Status</label>
                                    <select class="form-select" id="update-taskStatus">
                                        <option value="pending">Pending</option>
                                        <option value="in_progress">In Progress</option>
                                        <option value="completed">Completed</option>
                                    </select>
                        ...
                        <div class="modal-content">
                            <div class="modal-body text-center p-4">
                                ...
                                <div class="d-flex gap-2 justify-content-center">
                                    ...
                                    <button class="btn btn-danger" id="confirmDeleteTaskBtn">Delete</button>
                                </div>
                            </div>
                        </div>
                    </div>
...
function statusBadge(status) {
    const map = {
        ...
        in_progress: '<span class="badge bg-primary-subtle text-primary">In Progress</span>',
...
async function loadTasks() {
    try {
        ...
        const res = await fetch(window.API_URL + '/housekeeping', {
...
function updateStats(tasks) {
    ...
    document.getElementById('stat-pending').textContent = tasks.filter(t => t.cleaningStatus === 'pending').length;
    document.getElementById('stat-inprogress').textContent = tasks.filter(t => t.cleaningStatus === 'in_progress').length;
    document.getElementById('stat-completed').textContent = tasks.filter(t => t.cleaningStatus === 'completed').length;
}

function renderTasks(tasks) {
    const tbody = document.getElementById('housekeeping-table-body');
    if (!tasks.length) {
        tbody.innerHTML = '<tr><td colspan="7" class="text-center py-4 text-muted">No tasks found. Assign a cleaning task to get started.</td></tr>';
        return;
    }
    tbody.innerHTML = tasks.map((t, i) => {
        const room = t.roomId ? 'Room ' + t.roomId.roomNumber + ' (' + t.roomId.type + ')' : '—';
        const staff = t.assignedStaff?.name || '—';
        const assignedOn = new Date(t.createdAt).toLocaleDateString('en-PK');
        const completedAt = t.completedAt ? new Date(t.completedAt).toLocaleDateString('en-PK') : '—';
        return `<tr>
            <td>${i + 1}</td>
            <td><span class="fw-medium">${room}</span></td>
            <td>
                <div class="d-flex align-items-center gap-2">
                    <div class="avatar-xs">
                        <span class="avatar-title rounded-circle bg-success-subtle text-success" style="font-size:11px;">
                            ${staff.split(' ').map(n=>n[0]).join('').toUpperCase().slice(0,2)}
                        </span>
                    </div>
                    ${staff}
                </div>
            ...
            <td><small class="text-muted">${assignedOn}</small></td>
            <td><small class="text-muted">${completedAt}</small></td>
            <td>
                <div class="d-flex gap-1">
                    <button class="btn btn-soft-primary btn-sm" onclick="openUpdateStatus('${t._id}')" title="Update Status">
                        <i class="ri-edit-line"></i>
                    </button>
                    <button class="btn btn-soft-danger btn-sm" onclick="openDeleteTask('${t._id}')" title="Delete">
...
document.getElementById('filter-status').addEventListener('change', function() {
    ...
    renderTasks(status ? allTasks.filter(t => t.cleaningStatus === status) : allTasks);
});

// Load rooms and staff for modal
async function loadModalData() {
    ...
    document.getElementById('add-roomId').innerHTML =
        '<option value="">Select Room</option>' +
        rooms.map(r => `<option value="${r._id}">Room ${r.roomNumber} (${r.type}) — ${r.status}</option>`).join('');

    const hkStaff = users.filter(u => u.role === 'housekeeping');
    document.getElementById('add-staffId').innerHTML =
        '<option value="">Select Housekeeping Staff</option>' +
        hkStaff.map(u => `<option value="${u._id}">${u.name}</option>`).join('');
}

document.getElementById('addTaskModal').addEventListener('show.bs.modal', loadModalData);
...
document.getElementById('addTaskBtn').addEventListener('click', async function() {
    ...
    const roomId = document.getElementById('add-roomId').value;
    const staffId = document.getElementById('add-staffId').value;

    if (!roomId || !staffId) {
        errEl.textContent = 'Please select room and staff.';
    ...
    try {
        const res = await fetch(window.API_URL + '/housekeeping', {
            ...
            body: JSON.stringify({ roomId, assignedStaff: staffId })
        });
        ...
        if (res.ok) {
            ...
            errEl.textContent = data.message || 'Error assigning task.';
            errEl.classList.remove('d-none');
        }
    } catch(err) {
...
function openUpdateStatus(id) {
    ...
    if (!t) return;
    document.getElementById('update-taskId').value = id;
    document.getElementById('update-taskStatus').value = t.cleaningStatus;
    new bootstrap.Modal(document.getElementById('updateStatusModal')).show();
}

document.getElementById('updateStatusBtn').addEventListener('click', async function() {
    const btn = this;
    const id = document.getElementById('update-taskId').value;
    const status = document.getElementById('update-taskStatus').value;

    btn.disabled = true; btn.textContent = 'Updating...';
    try {
        const res = await fetch(window.API_URL + '/housekeeping/' + id, {
            method: 'PUT',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': 'Bearer ' + localStorage.getItem('token')
...
document.getElementById('confirmDeleteTaskBtn').addEventListener('click', async function() {
    const btn = this;
