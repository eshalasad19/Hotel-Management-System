...
const Feedback = () => {
  ...
  useEffect(() => {
    ...
    if (filterRating)  result = result.filter(f => f.rating === Number(filterRating));
    if (search) {
  ...
  const renderStars = (rating) => {
    return (
      <div className="d-flex gap-1">
        {[1,2,3,4,5].map(s => (
          <i key={s} className={`ri-star-${s <= rating ? 'fill' : 'line'}`}
  ...
  return (
    <div className="container-fluid">
      ...
      <div className="row mb-3">
        {[
          { label: 'Total Reviews',   value: feedbacks.length,                          icon: 'bx bx-message-dots', color: 'info'    },
          { label: 'Average Rating',  value: `${avg} / 5`,                              icon: 'bx bx-star',         color: 'warning' },
          { label: 'Positive (4-5⭐)', value: feedbacks.filter(f => f.rating >= 4).length, icon: 'bx bx-happy',    color: 'success' },
          { label: 'Negative (1-2⭐)', value: feedbacks.filter(f => f.rating <= 2).length, icon: 'bx bx-sad',      color: 'danger'  },
        ].map((s, i) => (
                ...
                {filtered.length === 0 ? (
                  ...
                  return (
                    <tr key={f._id}>
                      ...
                      <td>
                        <small className="text-muted" style={{ maxWidth: 220, display: 'block', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                          {f.review || '—'}
                        </small>
                      </td>
                      ...
                      <td>
                        <button
                          ...
                          onClick={() => { setSelected(f); setShowDetailModal(true); }}
                          title="View Detail"
                        >
            ...
            <div className="modal-content">
              ...
              <div className="modal-body">
                <div className="row g-3">
                    ...
                    <div className="p-3 bg-light rounded d-flex align-items-center gap-3">
                      <div className="avatar-sm">
                        <span className="avatar-title rounded-circle bg-primary text-white fw-bold">
                          {(selected.userId?.name || 'G').charAt(0).toUpperCase()}
                        </span>
                      </div>
                      <div>
                        <h6 className="mb-0 fw-semibold">{selected.userId?.name || 'Guest'}</h6>
                        <small className="text-muted">{selected.userId?.email || '—'}</small>
                      </div>
                    </div>
                  </div>
                  ...
                  {/* <div className="col-12">
                    <div className="p-3 bg-light rounded">
                      <p className="text-muted mb-1 fs-12">Reply Status</p>
                      {selected.adminReply ? (
                        <>
                          <span className="badge bg-success-subtle text-success mb-2">
                            <i className="ri-check-line me-1"></i>Replied
                          </span>
                          <p className="mb-0 text-success">{selected.adminReply}</p>
                          {selected.repliedAt && (
                            <small className="text-muted">{new Date(selected.repliedAt).toLocaleString('en-PK')}</small>
                          )}
                        </>
                      ) : (
                        <span className="badge bg-warning-subtle text-warning">
                          <i className="ri-time-line me-1"></i>No reply yet
                        </span>
                      )}
                    </div>
                  </div> */}
