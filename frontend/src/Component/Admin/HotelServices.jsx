...
const HotelServices = () => {
  ...
  const handleAdd = async () => {
    ...
    try {
      await axios.post(`${API_URL}/hotel-services`, buildFormData(), {
        headers,
      });
  ...
  const handleEdit = async () => {
    ...
    try {
      await axios.put(
        `${API_URL}/hotel-services/${selected._id}`,
        buildFormData(),
        { headers },
      );
  ...
  const handleToggleActive = async (service) => {
    try {
      ...
      await axios.put(`${API_URL}/hotel-services/${service._id}`, fd, {
        headers,
      });
      loadServices();
    } catch (err) {
                ...
                {filtered.length === 0 ? (
                  ...
                  filtered.map((s, i) => {
                    ...
                    return (
                      <tr
                        key={s._id}
                        className={!s.isActive ? "opacity-75" : ""}
                      >
                        <td>{i + 1}</td>
                        <td>
                          {getImageSrc(s) ? (
                        ...
                        <td>
                          <span
                            className={`badge ${s.price === "Free" ? "bg-success-subtle text-success" : "bg-primary-subtle text-primary"}`}
                          >
                            {s.price || "Free"}
                          </span>
                        </td>
