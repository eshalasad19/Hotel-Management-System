...
const Bookings = () => {
  ...
  useEffect(() => {
    let result = bookings;
    if (filterBookingStatus) result = result.filter(b => b.bookingStatus === filterBookingStatus);
    if (filterPaymentStatus) result = result.filter(b => b.paymentStatus === filterPaymentStatus);
    setFiltered(result);
  ...
  return (
    <div className="container-fluid">
      ...
      <div className="row mb-3">
        {[
          { label: 'Total Bookings', value: stats.total, icon: 'bx bx-calendar-check', color: 'info' },
          { label: 'Pending', value: stats.pending, icon: 'bx bx-time', color: 'warning' },
          { label: 'Confirmed', value: stats.confirmed, icon: 'bx bx-check-circle', color: 'success' },
          { label: 'Cancelled', value: stats.cancelled, icon: 'bx bx-x-circle', color: 'danger' },
        ].map((s, i) => (
      ...
      <div className="card">
        <div className="card-header d-flex align-items-center">
          ...
          <div className="d-flex gap-2">
            <select className="form-select form-select-sm w-auto" value={filterBookingStatus} onChange={e => setFilterBookingStatus(e.target.value)}>
            ...
            <select className="form-select form-select-sm w-auto" value={filterPaymentStatus} onChange={e => setFilterPaymentStatus(e.target.value)}>
            ...
            <table className="table table-hover table-nowrap align-middle mb-0">
              <thead className="table-light">
                <tr>
                  ...
                  <th>Check Out</th><th>Amount</th><th>Payment</th><th>Status</th><th>Actions</th>
                </tr>
              </thead>
              <tbody>
                {filtered.length === 0 ? (
                  <tr><td colSpan="9" className="text-center py-4 text-muted">No bookings found.</td></tr>
                ) : filtered.map((b, i) => (
                  <tr key={b._id}>
                    <td>{i + 1}</td>
                    <td>
            ...
            <div className="modal-content">
              ...
              <div className="modal-body">
                <div className="row g-3">
                  {[
                    { label: 'Guest Name', value: selectedBooking.userId?.name || 'Guest' },
                    { label: 'Email', value: selectedBooking.userId?.email || '—' },
                    { label: 'Phone', value: selectedBooking.userId?.phone || '—' },
                    { label: 'Room', value: selectedBooking.roomId ? `Room ${selectedBooking.roomId.roomNumber} (${selectedBooking.roomId.type})` : '—' },
                    { label: 'Check In', value: new Date(selectedBooking.checkInDate).toLocaleDateString('en-PK') },
                    { label: 'Check Out', value: new Date(selectedBooking.checkOutDate).toLocaleDateString('en-PK') },
                    { label: 'Guests', value: `${selectedBooking.guests} Person(s)` },
                    { label: 'Total Amount', value: `PKR ${Number(selectedBooking.totalAmount).toLocaleString()}` },
                  ].map((item, i) => (
                  ...
                  <div className="col-md-6">
                    <div className="p-3 bg-light rounded">
                      ...
                      <h6 className="mb-0">{statusBadge(selectedBooking.bookingStatus)}</h6>
                    </div>
                  </div>
                  <div className="col-md-6">
                    <div className="p-3 bg-light rounded">
                      <p className="text-muted mb-1 fs-12">Payment Status</p>
                      <h6 className="mb-0">{paymentBadge(selectedBooking.paymentStatus)}</h6>
                    </div>
                  </div>
                  {selectedBooking.specialRequests && (
                    <div className="col-12">
                      <div className="p-3 bg-light rounded">
                        <p className="text-muted mb-1 fs-12">Special Requests</p>
                        <h6 className="mb-0">{selectedBooking.specialRequests}</h6>
                      </div>
                    </div>
                  )}
                </div>
