...
<head>
    ...
    <meta charset="utf-8">
    <title>Payments & Invoices | Hotel Management System</title>
                ...
                <div class="container-fluid">
                    <div class="row">
                        <div class="col-12">
                            <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                                <h4 class="mb-sm-0">Payments & Invoices</h4>
                            </div>
                        </div>
                    </div>
                                ...
                                <div class="card-body">
                                    <div class="d-flex align-items-center">
                                        ...
                                        <div class="flex-grow-1 ms-3">
                                            <p class="text-uppercase fw-medium text-muted mb-1 fs-12">Total Revenue</p>
                                            <h4 class="mb-0 fw-semibold" id="stat-revenue">PKR 0</h4>
                                        </div>
                                    </div>
                                </div>
                            </div>
                                ...
                                <div class="card-body">
                                    <div class="d-flex align-items-center">
                                        ...
                                        <div class="flex-grow-1 ms-3">
                                            ...
                                            <h4 class="mb-0 fw-semibold" id="stat-paid">PKR 0</h4>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="card card-animate">
                                <div class="card-body">
                                    <div class="d-flex align-items-center">
                                        ...
                                        <div class="flex-grow-1 ms-3">
                                            <p class="text-uppercase fw-medium text-muted mb-1 fs-12">Unpaid</p>
                                            <h4 class="mb-0 fw-semibold" id="stat-unpaid">PKR 0</h4>
                                        </div>
                                    </div>
                                </div>
                            </div>
                    ...
                    <div class="card">
                        <div class="card-header d-flex align-items-center">
                            <h5 class="card-title mb-0 flex-grow-1">All Invoices</h5>
                            <div class="d-flex gap-2">
                                <select class="form-select form-select-sm w-auto" id="filter-payment">
                ...
                <div class="modal fade" id="updatePaymentModal" tabindex="-1">
                    <div class="modal-dialog modal-sm">
                        <div class="modal-content">
                            ...
                            <div class="modal-body">
                                <input type="hidden" id="update-paymentBookingId">
                                <div class="mb-3">
                            ...
                            <div class="modal-footer">
                                ...
                                <button type="button" class="btn btn-success" id="confirmUpdatePaymentBtn">Update</button>
                            </div>
                        </div>
                    </div>
                </div>
...
function updateStats(bookings) {
    ...
    const paid = bookings.filter(b => b.paymentStatus === 'paid').reduce((s, b) => s + b.totalAmount, 0);
    const unpaid = bookings.filter(b => b.paymentStatus === 'unpaid').reduce((s, b) => s + b.totalAmount, 0);
    document.getElementById('stat-revenue').textContent = formatPKR(total);
    document.getElementById('stat-paid').textContent = formatPKR(paid);
    document.getElementById('stat-unpaid').textContent = formatPKR(unpaid);
...
function renderPayments(bookings) {
    ...
    tbody.innerHTML = bookings.map((b, i) => {
        ...
        return `<tr>
            <td><span class="fw-medium text-primary">${invoiceNo}</span></td>
            <td>
                <div class="fw-medium">${guest}</div>
                <small class="text-muted">${b.userId?.phone || ''}</small>
            </td>
            <td>${room}</td>
            <td>${checkIn}</td>
            <td>${checkOut}</td>
            <td>${nights} night(s)</td>
            <td class="fw-semibold">${formatPKR(b.totalAmount)}</td>
            <td>${paymentBadge(b.paymentStatus)}</td>
            <td>
                <div class="d-flex gap-1">
                    <button class="btn btn-soft-info btn-sm" onclick="viewInvoice('${b._id}', '${invoiceNo}')" title="View Invoice">
                        <i class="ri-eye-line"></i>
                    </button>
                    <button class="btn btn-soft-primary btn-sm" onclick="openUpdatePayment('${b._id}')" title="Update Payment">
...
document.getElementById('filter-payment').addEventListener('change', function() {
    ...
    renderPayments(status ? allBookings.filter(b => b.paymentStatus === status) : allBookings);
});
...
function viewInvoice(id, invoiceNo) {
    ...
    if (!b) return;
    currentInvoiceBooking = b;

    const guest = b.userId?.name || 'Guest';
    const phone = b.userId?.phone || '—';
    const email = b.userId?.email || '—';
    const room = b.roomId ? 'Room ' + b.roomId.roomNumber : '—';
    const roomType = b.roomId?.type || '—';
    const pricePerNight = b.roomId?.price || 0;
    const checkIn = new Date(b.checkInDate).toLocaleDateString('en-PK');
    const checkOut = new Date(b.checkOutDate).toLocaleDateString('en-PK');
    const nights = Math.ceil((new Date(b.checkOutDate) - new Date(b.checkInDate)) / (1000*60*60*24));
    const invoiceDate = new Date(b.createdAt).toLocaleDateString('en-PK');
    ...
    document.getElementById('invoice-body').innerHTML = `
        <div id="printable-invoice">
            ...
            <div class="d-flex justify-content-between align-items-start mb-4">
                <div>
                    <h4 class="fw-bold mb-1">🏨 Hotel Management System</h4>
                    <p class="text-muted mb-0">Official Invoice</p>
                </div>
                <div class="text-end">
                    <h5 class="fw-bold text-primary">${invoiceNo}</h5>
                    <p class="text-muted mb-0">Date: ${invoiceDate}</p>
                    <div class="mt-1">${paymentBadge(b.paymentStatus)}</div>
                </div>
            </div>
            ...
            <div class="row g-3 mb-4">
                <div class="col-md-6">
                    <div class="p-3 bg-light rounded">
                        <h6 class="fw-semibold mb-3 text-primary">Guest Information</h6>
                        <p class="mb-1"><span class="text-muted">Name:</span> <strong>${guest}</strong></p>
                        <p class="mb-1"><span class="text-muted">Phone:</span> ${phone}</p>
                        <p class="mb-0"><span class="text-muted">Email:</span> ${email.includes('@walkin.hotel') ? '—' : email}</p>
                    </div>
                </div>
                <div class="col-md-6">
                    <div class="p-3 bg-light rounded">
                        <h6 class="fw-semibold mb-3 text-primary">Room Details</h6>
                        <p class="mb-1"><span class="text-muted">Room:</span> <strong>${room}</strong></p>
                        <p class="mb-1"><span class="text-muted">Type:</span> ${roomType.charAt(0).toUpperCase() + roomType.slice(1)}</p>
                        <p class="mb-0"><span class="text-muted">Price/Night:</span> ${formatPKR(pricePerNight)}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="row g-3 mb-4">
                <div class="col-md-4">
                    <div class="p-3 bg-light rounded text-center">
                        ...
                        <h6 class="fw-semibold text-success">${checkIn}</h6>
                    </div>
                </div>
                <div class="col-md-4">
                    <div class="p-3 bg-light rounded text-center">
                        <p class="text-muted mb-1 fs-12">CHECK OUT</p>
                        <h6 class="fw-semibold text-warning">${checkOut}</h6>
                    </div>
                </div>
                <div class="col-md-4">
                    <div class="p-3 bg-light rounded text-center">
                        <p class="text-muted mb-1 fs-12">DURATION</p>
                        <h6 class="fw-semibold text-info">${nights} Night(s)</h6>
                    </div>
                </div>
            </div>
            ...
            <div class="table-responsive mb-3">
                <table class="table table-bordered">
                    ...
                    <tbody>
                        <tr>
                            <td>${room} — ${roomType.charAt(0).toUpperCase() + roomType.slice(1)} Room</td>
                            <td class="text-center">${nights}</td>
                            <td class="text-end">${formatPKR(pricePerNight)}</td>
                            <td class="text-end fw-medium">${formatPKR(nights * pricePerNight)}</td>
                        </tr>
                        ${b.specialRequests ? `<tr><td colspan="4" class="text-muted fs-12">Special Request: ${b.specialRequests}</td></tr>` : ''}
                    </tbody>
                    <tfoot>
                        <tr class="table-light">
                            <td colspan="3" class="text-end fw-bold">Total Amount</td>
                            <td class="text-end fw-bold text-success fs-16">${formatPKR(b.totalAmount)}</td>
                        </tr>
                        <tr>
                            <td colspan="3" class="text-end text-muted">Payment Status</td>
                            <td class="text-end">${paymentBadge(b.paymentStatus)}</td>
                        </tr>
                    </tfoot>
                </table>
            </div>

            <div class="text-center text-muted fs-12 mt-3">
                <p class="mb-0">Thank you for choosing our hotel. We hope to see you again!</p>
...
function openUpdatePayment(id) {
    const b = allBookings.find(b => b._id === id);
    if (!b) return;
    document.getElementById('update-paymentBookingId').value = id;
...
document.getElementById('confirmUpdatePaymentBtn').addEventListener('click', async function() {
    const btn = this;
    const id = document.getElementById('update-paymentBookingId').value;
